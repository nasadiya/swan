...
button_response = st.button('Load')
if button_response:
        ...
        for trade in trade_data_parsed:
            if trade['trade'] == "Yes":
                ...
                with col2:
                    ...
                    options = ["None", "CS", "SDG"]
                    selected_status = st.selectbox("Trader : ", options=options, index='None' if trader not in options else options.index(trader), disabled=disable, key=trade['tradeid'])

                    # Store the selected trade status in session state
                    st.session_state.trade_status[trade['tradeid']] = selected_status
